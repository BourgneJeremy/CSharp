...
namespace WeatherPondClass.Controller
{
    public class Menu
    {
        public void Launch()
        ...
        public List<WeatherItem> PointsToGraphic(List<WeatherItem> weatherItems)
        {
            List<WeatherItem> itemsForGraphic = new List<WeatherItem>();
            int currentPoint = 0;

            int pointsToDisplay = 30;
            int totalPoints = weatherItems.Count;

            for (int i = 0; i < pointsToDisplay; i++)
            {
                currentPoint += totalPoints / 30;
                // Console.WriteLine("i: " + currentPoint + ", content: " + weatherItems[currentPoint]);
                itemsForGraphic.Add(weatherItems[currentPoint]);
                // Ajouter les items à la courbe
            }
            return itemsForGraphic;
        }
    }
}
